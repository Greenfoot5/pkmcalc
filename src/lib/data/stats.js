...
export let allPrices = derived([zeal, bargaining, manaControl, zealousConviction,
    ...
    ([$zeal, $bargaining, $manaControl, $zealousConviction,
         $skillEffect]) =>
        (($zeal - 1) * skills.theOrder[1].effectValue + 1) *
        (($bargaining - 1) * skills.fundamentals[2].effectValue + 1) *
        (($manaControl - 1) * skills.magic[0].effectValue + 1) *
        (($zealousConviction - 1) * skills.darkMagic[2].effectValue + 1) *
        $skillEffect);
...
export let greed = derived([materialism, blazingFervour, skillEffect],
    ([$materialism, $blazingFervour, $skillEffect]) =>
        (($materialism - 1) * skills.magic[3].effectValue + 1) *
        (($blazingFervour - 1) * skills.darkMagic[7].effectValue + 1) *
        $skillEffect);
...
export let fanaGain = derived([lifeEssence, ardentBelief, absoluteFaith, skillEffect],
    ([$lifeEssence, $ardentBelief, $absoluteFaith, $skillEffect]) =>
        (($lifeEssence - 1) * skills.magic[1].effectValue + 1) *
        (($ardentBelief - 1) * skills.darkMagic[1].effectValue + 1) *
        (($absoluteFaith - 1) * skills.darkMagic[4].effectValue + 1) *
        $skillEffect);
